...
1.環境変数の設定: GitHubリポジトリの Settings > Secrets and variables > Actions に以下を設定してください。
  ・OPENAI_API_KEY: OpenAIのAPIキー (sk-...)
2.スケジュールの調整: .github/workflows/auto_post.yml の cron を変更することで実行時間を調整可能です。
  ・デフォルト: 0 22 * * * (UTC 22:00 = 日本時間 07:00)
3.手動実行: GitHubの Actions タブから「Run workflow」ボタンを押すことで、即座に記事生成をテストできます。
